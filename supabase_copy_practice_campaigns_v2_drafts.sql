-- copy_practice_campaigns_v2 (drafts variant)
-- Catalog rows with a chosen creative on source -> 'inProgress' (with notification, as before).
-- Catalog rows without a chosen creative on source -> 'draft' (was 'onPlan').
-- Bespoke rows always -> 'inProgress' (unchanged).
...
BEGIN
  ...
  LOOP
    ...
    -- Only fire notification + history when going to inProgress.
  ...
  -- Bespoke campaigns + their selections (always -> inProgress)
  FOR bc IN
    SELECT * FROM public.bespoke_campaigns b
    WHERE b.practice_id = v_src
  LOOP
    IF EXISTS (
      SELECT 1 FROM public.bespoke_campaigns t
      WHERE t.practice_id = v_dst AND LOWER(t.name) = LOWER(bc.name)
    ) THEN
      CONTINUE;
    END IF;

    INSERT INTO public.bespoke_campaigns (
      practice_id, name, description, category, objectives, topics,
      assets, reference_links, source, event_type, requirements,
      creatives, created_by
    )
    VALUES (
      v_dst, bc.name, bc.description, bc.category, bc.objectives, bc.topics,
      bc.assets, bc.reference_links, 'copied', bc.event_type, bc.requirements,
      bc.creatives, bc.created_by
    )
    RETURNING id INTO new_bespoke_id;
    copied_bespoke := copied_bespoke + 1;

    SELECT s.* INTO src_sel
    FROM public.selections s
    WHERE s.bespoke_campaign_id = bc.id AND s.practice_id = v_src
    LIMIT 1;

    v_chosen_url := COALESCE(src_sel.assets->>'creative',
                             'https://cdn.hakimgroup.io/digE8/LEquQaPE75.png/raw');
    v_chosen_label := 'Bespoke - ' || bc.name;
    ...
    VALUES (
      v_dst, NULL, bc.created_by,
      COALESCE(src_sel.assets, jsonb_build_object('creative', v_chosen_url)),
      COALESCE(src_sel.reference_links, bc.reference_links),
      'copied',
      COALESCE(src_sel.from_date, current_date),
      COALESCE(src_sel.to_date, current_date + interval '14 days'),
      'inProgress',
      COALESCE(src_sel.notes, bc.description),
      true, new_bespoke_id
    )
    RETURNING id INTO new_selection_id;
    ...
    v_payload := jsonb_build_object(
      'name', bc.name, 'category', COALESCE(bc.category, 'Campaign'),
      'description', bc.description, 'event_type', bc.event_type,
      'from_date', COALESCE(src_sel.from_date, current_date),
      'to_date', COALESCE(src_sel.to_date, current_date + interval '14 days'),
      'campaign_id', new_bespoke_id, 'bespoke_id', new_bespoke_id, 'is_bespoke', true,
      'chosen_creative', v_chosen_url, 'chosen_creative_label', v_chosen_label,
      'assets', COALESCE(src_sel.assets, jsonb_build_object('creative', v_chosen_url)),
      'creatives', COALESCE(bc.creatives, '[]'::jsonb),
      'note', COALESCE(src_sel.notes, bc.description),
      'reference_links', COALESCE(bc.reference_links, '[]'::jsonb),
      'original_notes', COALESCE(src_sel.notes, bc.description),
      'requirements', bc.requirements, 'copied_from', v_src
    );
    ...
    VALUES (
      'inProgress', new_selection_id, NULL, v_dst, v_actor, 'admins',
      'Bespoke ' || COALESCE(LOWER(bc.category), 'campaign') || ' copied',
      'Practice copied a bespoke ' || COALESCE(LOWER(bc.category), 'campaign') || ' from another practice.',
      v_payload
    )
    RETURNING id INTO v_notification_id;
    ...
    VALUES (
      new_selection_id, NULL, 'inProgress', v_actor,
      'Bespoke copied from another practice with asset choices.',
      jsonb_build_object('id', v_dst, 'name', v_practice.name)
    );
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'copied_bespoke_campaigns', copied_bespoke
  );
END;
$$;
