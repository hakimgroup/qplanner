-- request_assets v3: send ALL assets to practice (not just requested subset)
-- Mark admin-selected assets with adminRequested=true, reset userSelected=false
-- so the practice can choose from the full catalog.

-- 1) Create a helper that marks admin-requested items and resets userSelected
CREATE OR REPLACE FUNCTION public.assets_prepare_for_practice(p_assets jsonb)
RETURNS jsonb
LANGUAGE sql
STABLE
AS $function$
  with groups as (
    select unnest(array['printedAssets','digitalAssets','externalPlacements']) as k
  ),
  transformed as (
    select k,
           coalesce((
             select jsonb_agg(
               itm
               -- Mark adminRequested if userSelected was true
               || jsonb_build_object(
                    'adminRequested',
                    coalesce((itm ->> 'userSelected')::boolean, false)
                  )
               -- Reset userSelected so practice starts fresh
               || jsonb_build_object('userSelected', false)
             )
             from jsonb_array_elements(coalesce(p_assets -> k, '[]'::jsonb)) as e(itm)
           ), '[]'::jsonb) as arr
    from groups
  )
  select jsonb_build_object(
    'printedAssets',      (select arr from transformed where k='printedAssets'),
    'digitalAssets',      (select arr from transformed where k='digitalAssets'),
    'externalPlacements', (select arr from transformed where k='externalPlacements')
  );
$function$;

-- 2) Update request_assets to use the new helper instead of assets_requested_subset
...
BEGIN
  ...
  --------------------------------------------------------------------
  -- 5. Prepare ALL assets for practice:
  --    mark adminRequested=true where admin selected, reset userSelected=false
  --------------------------------------------------------------------
  v_assets_prepared := public.assets_prepare_for_practice(v_assets_src);
