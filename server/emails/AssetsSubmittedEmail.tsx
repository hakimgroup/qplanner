...
export const AssetsSubmittedEmail = ({
	practiceName,
	practiceId,
	campaignName,
	campaignCategory,
	...
	const printedWithQty = assets?.printedAssets?.filter(a => a.quantity && a.quantity > 0) || [];
	const externalWithQty = assets?.externalPlacements?.filter(a => a.quantity && a.quantity > 0) || [];
	const totalAssets = digitalWithQty.length + printedWithQty.length + externalWithQty.length;

	return (
		<Html>
			<Head />
			<Preview>
				{practiceName} submitted choices for {campaignName}
			</Preview>
			<Tailwind
				config={{
					theme: {
				...
				<Body className="bg-offwhite text-base font-sans">
					...
					<Container className="bg-white p-45">
						<Section style={badgeContainer}>
							<span style={badge}>Submission Received</span>
						</Section>

						<Heading className="text-center my-0 leading-8">
							Practice Submitted Choices
						</Heading>

						<Section>
							<Text className="text-base">
								<Link className="text-brand font-bold">{practiceName}</Link> has
								submitted their creative selection and asset choices for the following campaign:
							</Text>
						</Section>
						...
						{totalAssets > 0 && (
							<Section>
								<Text style={sectionTitle}>Asset Selections</Text>

								{digitalWithQty.length > 0 && (
									<>
										<Text style={assetCategoryLabel}>Digital Assets</Text>
										<ul style={assetList}>
											{digitalWithQty.map((asset, i) => (
												<li key={`d-${i}`} style={assetItem}>
													{asset.name || asset.label}: <strong>{asset.quantity}</strong>
												</li>
											))}
										</ul>
									</>
								...
								{printedWithQty.length > 0 && (
									<>
										<Text style={assetCategoryLabel}>Printed Assets</Text>
										<ul style={assetList}>
											{printedWithQty.map((asset, i) => (
								...
								{externalWithQty.length > 0 && (
									<>
										<Text style={assetCategoryLabel}>External Placements</Text>
										<ul style={assetList}>
											{externalWithQty.map((asset, i) => (
						...
						<Section>
							<Text style={paragraph}>
								Please review the submission and proceed with the next steps in the workflow.
							</Text>
						</Section>
					</Container>
...
export default AssetsSubmittedEmail;
