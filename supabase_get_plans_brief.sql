...
-- Surface the structured `brief` (narrative + uploaded file refs) on
-- bespoke rows returned by get_plans, so the plan/selection detail can
-- render it first-class. Additive: one new `brief` key per row (NULL for
-- non-bespoke). Returns jsonb (no RETURNS TABLE signature) so CREATE OR
-- REPLACE is safe.
--
-- Apply to STAGING first, verify, then PROD. Idempotent.
-- =====================================================================

CREATE OR REPLACE FUNCTION public.get_plans(p_practice_ids uuid[] DEFAULT NULL::uuid[], p_status text DEFAULT NULL::text, p_category text DEFAULT NULL::text, p_source text DEFAULT NULL::text, p_tier text DEFAULT NULL::text, p_is_bespoke boolean DEFAULT false, p_limit integer DEFAULT NULL::integer, p_offset integer DEFAULT 0, p_search text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
...
base_rows AS (
  SELECT
    ...
    CASE
      WHEN s.status IN ('onPlan', 'draft')
        THEN COALESCE(c.creatives, bc.creatives, '[]'::jsonb)
      ELSE NULL
    END                                                                 AS creatives,
    s.markup_link                                                       AS markup_link,
    s.assets_link                                                       AS assets_link,
    s.markup_opened_at                                                  AS markup_opened_at
  ...
  WHERE
    ...
    -- Drafts are practice-only: hide them from admins / super_admins.
    AND (
      s.status <> 'draft'
      OR EXISTS (
        SELECT 1 FROM public.allowed_users au
        WHERE au.id = auth.uid()
          AND LOWER(COALESCE(au.role, '')) NOT IN ('admin', 'super_admin')
      )
    )
),
meta AS (
