-- =====================================================================
-- Bespoke form revamp — Phase 2
-- Add `brief` to get_campaigns so the practice-facing campaign selector /
-- View drawer can render the structured bespoke brief (get_campaigns is the
-- source for that drawer's `c`, not get_plans). Appended as the last column;
-- catalog rows get NULL, bespoke rows get bc.brief.
--
-- RETURNS TABLE signature changes -> DROP then CREATE.
-- Apply to STAGING first, verify, then PROD. Idempotent.
-- =====================================================================

DROP FUNCTION IF EXISTS public.get_campaigns(uuid);

CREATE OR REPLACE FUNCTION public.get_campaigns(p_practice uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, name text, description text, custom_events boolean, category text, tier text, assets jsonb, creatives jsonb, objectives jsonb, topics jsonb, availability jsonb, reference_links jsonb, status text, source text, is_bespoke boolean, selected boolean, selection_id uuid, selection_practice_id uuid, selection_practice_name text, selection_from_date date, selection_to_date date, notes text, bespoke_campaign_id uuid, is_event boolean, event_type text, requirements text, markup_link text, assets_link text, self_print boolean, brief jsonb)
 LANGUAGE sql
...
bespoke_rows as (
  select
   ...
   ,coalesce(bc.brief, '{}'::jsonb)                     as brief
