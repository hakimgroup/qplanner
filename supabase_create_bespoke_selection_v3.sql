-- =====================================================================
-- Bespoke form revamp — Phase 0.4
-- create_bespoke_selection_v3 — sibling of v2 (v2 left intact for
-- rollback per the v1/v2 convention). Adds the structured `p_brief`:
--   * persisted on bespoke_campaigns.brief
--   * mirrored into notifications.payload.brief  (the Trello/n8n contract)
--
-- Everything else is a faithful port of v2:
--   * inserts selection at inProgress, auto-applies DEFAULT bespoke
--     creative when none chosen (the form no longer asks for a creative),
--   * builds the inProgress notification (fires the `n8n` INSERT trigger),
--     targets practice admins + global super_admins, writes status history.
--
-- The client composes a human-readable `p_description` from the brief for
-- back-compat, so existing surfaces that read `description` still work.
--
-- Apply to STAGING first, verify, then PROD. Idempotent (CREATE OR REPLACE
-- of a NEW name — no signature clash with v2).
-- =====================================================================

CREATE OR REPLACE FUNCTION public.create_bespoke_selection_v3(
  p_practice        uuid,
  p_name            text,
  p_description     text,
  p_from_date       date,
...
BEGIN
  ...
  v_default_creatives := jsonb_build_array(
    jsonb_build_object(
      'label', 'Bespoke - ' || p_name,
      'url',   v_default_creative_url
    )
  );

  v_chosen_url := COALESCE(p_chosen_creative, v_default_creative_url);
  v_chosen_label := 'Bespoke - ' || p_name;
  ...
  INSERT INTO public.bespoke_campaigns (
    ...
    assets, reference_links, creatives, created_by, brief
  )
  VALUES (
    p_practice, p_name, p_description, 'Campaign',
  ...
  INSERT INTO public.selections (
    practice_id, bespoke_campaign_id, from_date, to_date, status,
    bespoke, notes, assets, reference_links, updated_at, created_by
  )
  VALUES (
    p_practice, v_bc_id, p_from_date, p_to_date, 'inProgress',
    true, p_notes, v_assets_to_save,
    COALESCE(p_reference_links, '[]'::jsonb),
    NOW(), v_actor
  )
  ...
  v_payload := jsonb_build_object(
    ...
    'bespoke_id',            v_bc_id,
    'is_bespoke',            true,
    'chosen_creative',       v_chosen_url,
    'chosen_creative_label', v_chosen_label,
    'assets',                v_assets_to_save,
    'creatives',             v_default_creatives,
    'note',                  p_notes,
    'reference_links',       COALESCE(p_reference_links, '[]'::jsonb),
    'original_notes',        p_notes,
    'requirements',          NULL,
  ...
  VALUES (
    'inProgress', v_sel_id, NULL, p_practice, v_actor,
    'admins',
    'Bespoke campaign added',
    'Practice added a bespoke campaign with their asset choices.',
    v_payload
  )
  ...
  VALUES (
    ...
    'Practice added bespoke campaign with their asset choices.',
    v_recipients_json,
...
GRANT EXECUTE ON FUNCTION public.create_bespoke_selection_v3(
  uuid, text, text, date, date, text, jsonb, jsonb, jsonb, jsonb, text, jsonb, jsonb
) TO authenticated;
