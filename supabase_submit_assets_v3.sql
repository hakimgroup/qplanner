-- submit_assets v3: persist chosen_creative inside the assets JSONB on the selection row
...
BEGIN
  ...
  --------------------------------------------------------------------
  -- 4) Update selection → inProgress and persist assets + chosen creative
  --------------------------------------------------------------------
  v_final_assets := COALESCE(p_assets, '{}'::jsonb)
    || jsonb_build_object('creative', p_chosen_creative);

  UPDATE public.selections
     SET status = 'inProgress',
         assets = v_final_assets
   WHERE id = v_sel.id;
