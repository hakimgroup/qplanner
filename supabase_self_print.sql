-- Add self_print column to selections and archived_selections
ALTER TABLE public.selections
  ADD COLUMN IF NOT EXISTS self_print boolean NOT NULL DEFAULT false;

ALTER TABLE public.archived_selections
  ADD COLUMN IF NOT EXISTS self_print boolean NOT NULL DEFAULT false;

-- Update confirm_assets to accept and store self_print
DROP FUNCTION IF EXISTS public.confirm_assets(uuid, text);

CREATE OR REPLACE FUNCTION public.confirm_assets(
  p_selection_id uuid,
  p_note text DEFAULT NULL,
  p_self_print boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 3. Verify user has access to this practice (admins can act on behalf of any practice)
  ...
  -- 5. Update selection status to 'confirmed' and set self_print
  UPDATE selections
  SET
    status = 'confirmed',
  ...
  -- 8. Create notification_targets for all admins
  ...
  -- 9. Build recipients array for status history
  SELECT COALESCE(
           jsonb_agg(
             jsonb_build_object(
  ...
  -- 10. Log the status transition with recipients
