...
export default function CampaignModal({ opened, onClose, row }: Props) {
  ...
  const form = useForm({
    initialValues: {
      ...
      topics: row?.topics ?? [],
      from: row?.availability?.from ? new Date(row.availability.from) : null,
    ...
    upsert(
      {
        ...
        availability:
          vals.from && vals.to
            ? ({ from: vals.from, to: vals.to } as any)
            : null,
        reference_links,
      },
      { onSuccess: onClose }
      ...
      <form onSubmit={handleSubmit}>
        <Stack gap="md">
          ...
          <MultiSelect
            ...
            value={Array.isArray(form.values.topics) ? form.values.topics : []}
              ...
              <Grid key={idx} gutter="xs" align="center">
                <Grid.Col span={links.length > 1 ? 11 : 12}>
                  <TextInput
                    ...
                    value={link}
                    onChange={(e) =>
                      handleChangeLink(idx, e.currentTarget.value)
                    }
                  />
                </Grid.Col>
                {links.length > 1 && (
