    ...
    >
      <Stack gap={20}>
          ...
          <Stack mt={10} gap={10}>
            {row?.reference_links?.map((rl: string, i: number) => (
              <StyledButton
                ...
                key={rl}
                link={rl}
                leftSection={<IconShare3 size={18} color={T.gray[9]} />}
              >
                {getReferenceLinkLabel(rl, i)}
              </StyledButton>
            ))}

            {!row?.reference_links?.length && <IconMinus size={20} />}
          </Stack>
        </Stack>
        ...
        <Stack gap={10}>
          ...
          {row?.status === SelectionStatus.OnPlan && (
            <BulkRequestButton selectionIds={[row?.id]} />
          )}
        </Stack>
      </Stack>
    </Drawer>
