              ...
              {row?.assigned_practices.map((p, i) => (
                <Stack mt={i === 0 ? 0 : 15} gap={5} key={p.id}>
                  <Flex align="center" gap={5}>
                    <IconCircleFilled size={10} color={T.red[4]} />
                    <Text size="sm" fw={500}>
                      {p.name}
                    </Text>
                  </Flex>
                  {p.numberOfPlans > 0 && (
                    <Text size="xs" fw={500} c="green.9">
                      {p.numberOfPlans} planned activities already under this
                      practice
                    </Text>
                  )}
                </Stack>
              ...
              <Stack mt={"xs"}>
                {filteredPractices.length === 0 ? (
                  ...
                  filteredPractices.map((p, i) => {
                    const numberOfPlans = find(row?.assigned_practices, {
                      id: p.id,
                    })?.numberOfPlans;
                    return (
                      <Checkbox
                        ...
                        label={
                          <Stack gap={0} mt={-10}>
                            ...
                            {numberOfPlans > 0 && (
                              <Text size="xs" fw={500} c="green.9">
                                {numberOfPlans} planned activities already under
                                this practice
                              </Text>
                            )}
                          </Stack>
        ...
        {!isView && (
          <Button
            ...
            onClick={() => {
              ...
              if (editingRole && roleValue && roleValue !== row.role) {
