...
const PeopleAccessTable = ({ users, loading }: Props) => {
	...
	const openEdit = useCallback((data: AllowedUser) => {
		setRow(data);
		setMode("edit");
	}, []);
	const openView = useCallback((data: AllowedUser) => {
		setRow(data);
		setMode("view");
	}, []);
	const openDelete = useCallback((id: string) => setUserIdToDelete(id), []);
	...
	const colDefs: ColDef[] = useMemo(
		...
		[T.blue, T.lime, role, openEdit, openView, openDelete]
	);
		...
		<>
			...
			<DeleteUserModal
				...
				userName={
					userIdToDelete
						? (() => {
								const u = users.find((u) => u.id === userIdToDelete);
								return u
									? `${upperFirst(u.first_name)} ${upperFirst(u.last_name)}`
									: "";
						  })()
						: ""
				}
